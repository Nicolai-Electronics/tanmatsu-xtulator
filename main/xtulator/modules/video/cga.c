...
const uint8_t cga_palette[16][3] = {
    // R, G, B
...
int cga_init() {
    ...
    if (utility_loadFile(cga_font, 4096, "/sd/xtulator/video/cgachar.bin")) {
        ...
        case CGA_MODE_TEXT_40X25:
            ...
            for (scy = start_y; scy <= end_y; scy++) {
                ...
                for (scx = start_x; scx <= end_x; scx += 2) {
                    ...
                    cga_framebuffer[scy * CGA_HOR_RES + scx + 1] =
                        cga_framebuffer[scy * CGA_HOR_RES + scx];  // double pixels horizontally
                }
            }
            break;
