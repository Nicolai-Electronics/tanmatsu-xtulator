...
volatile uint8_t  vga_wmode, vga_rmode, vga_shiftmode, vga_rotate, vga_logicop, vga_enableplane, vga_readmap,
    vga_scandbl, vga_hdbl, vga_bpp, vga_latch[4];
...
int vga_init() {
    ...
    vga_hblankTimer    = timing_addTimer(vga_hblankCallback, NULL, 10000,
                                         TIMING_ENABLED);  // nonsense frequency values to begin with is fine
    vga_hblankEndTimer = timing_addTimer(vga_hblankEndCallback, NULL, 100, TIMING_ENABLED);  // same here
    vga_curScanline    = 0;

    for (i = 0; i < 4;
         i++) {  // 4 planes of 64 KB (It's actually 64K addresses on a 32-bit data bus on real VGA hardware)
    ...
    if (utility_loadFile(VBIOS, 32768, "/sd/xtulator/video/et4000.bin")) {
        return -1;
    }
...
void vga_updateScanlineTiming() {
    ...
    vga_hblankend   = ((uint64_t)vga_crtcd[0x02] * (uint64_t)vga_dots) +
                    (((uint64_t)vga_crtcd[0x03] & 0x1F) + 1) * (uint64_t)vga_dots;
    vga_hblanklen   = vga_hblankend - vga_hblankstart;
    vga_vblankstart = (uint64_t)vga_crtcd[0x10] | ((uint64_t)(vga_crtcd[0x07] & 0x04) << 6) |
                      ((uint64_t)(vga_crtcd[0x07] & 0x80) << 2);
    vga_vblankend = (uint64_t)vga_crtcd[0x06] | ((uint64_t)(vga_crtcd[0x07] & 0x01) << 8) |
                    ((uint64_t)(vga_crtcd[0x07] & 0x20) << 4);
    ...
    pixelclock = (double)timing_getFreq() /
                 pixelclock;  // get ratio of pixel clock vs our timer frequency for interval calculations
    ...
    /*printf("hblank start = %llu, hblank end = %llu, hblank len = %llu, disp interval = %llu, hblank interval = %llu,
    freq = %llu\r\n", vga_hblankstart, vga_hblankend, vga_hblanklen, vga_dispinterval, vga_hblankinterval, timing_freq);
    printf("vblank start = %llu, vblank end = %llu, vblank len = %llu, vblank interval = %llu, frameinterval =
...
void vga_update(uint32_t start_x, uint32_t start_y, uint32_t end_x, uint32_t end_y) {
    ...
    if (vga_attrd[0x10] & 1) {  // graphics mode enable
        ...
        debug_log(
            DEBUG_DETAIL,
            "[VGA] Resolution: %lux%lu %lu bpp (X stride: %lu, V lines per pixel: %lu, H lines per pixel = %lu)\r\n",
            ...
            for (scy = start_y; scy <= end_y; scy++) {
                ...
                for (scx = start_x; scx <= end_x; scx++) {
                    ...
                    if ((y == cursor_y) && (x == cursor_x) &&
                        ...
                        ((uint8_t)(scy % 16) <= (vga_crtcd[VGA_REG_DATA_CURSOR_END] & 31)) && vga_cursor_blink_state &&
                        cursorenable) {  // cursor should be displayed
                        ...
                        if (blinkenable && blink && !vga_cursor_blink_state) {
                            fontdata = 0;  // all pixels in character get background color if blink attribute set and
                                           // blink visible state is false
                        }
    ...
    if (vga_rmode == 0) {
        ...
        for (plane = 0; plane < 4; plane++) {
            if (vga_gfxd[0x07] & (1 << plane)) {  // color don't care bit check
                if ((vga_RAM[plane][addr] & 0x0F) ==
                    (vga_gfxd[0x02] & 0x0F)) {  // compare RAM value with color compare register
...
void vga_dumpregs() {
    ...
    for (i = 0; i < 0x15; i++) {
        debug_log(DEBUG_DETAIL, "  ATTR[0x%02X] = %u%u%u%u%u%u%u%u\r\n", i, (vga_attrd[i] >> 7) & 1,
                  (vga_attrd[i] >> 6) & 1, (vga_attrd[i] >> 5) & 1, (vga_attrd[i] >> 4) & 1, (vga_attrd[i] >> 3) & 1,
                  (vga_attrd[i] >> 2) & 1, (vga_attrd[i] >> 1) & 1, (vga_attrd[i] >> 0) & 1);
    }
    debug_log(DEBUG_DETAIL, "\r\n");
    for (i = 0; i < 0x05; i++) {
        debug_log(DEBUG_DETAIL, "  SEQ[0x%02X] = %u%u%u%u%u%u%u%u\r\n", i, (vga_seqd[i] >> 7) & 1,
                  (vga_seqd[i] >> 6) & 1, (vga_seqd[i] >> 5) & 1, (vga_seqd[i] >> 4) & 1, (vga_seqd[i] >> 3) & 1,
                  (vga_seqd[i] >> 2) & 1, (vga_seqd[i] >> 1) & 1, (vga_seqd[i] >> 0) & 1);
    }
    debug_log(DEBUG_DETAIL, "\r\n");
    for (i = 0; i < 0x09; i++) {
        debug_log(DEBUG_DETAIL, "  GFX[0x%02X] = %u%u%u%u%u%u%u%u\r\n", i, (vga_gfxd[i] >> 7) & 1,
                  (vga_gfxd[i] >> 6) & 1, (vga_gfxd[i] >> 5) & 1, (vga_gfxd[i] >> 4) & 1, (vga_gfxd[i] >> 3) & 1,
                  (vga_gfxd[i] >> 2) & 1, (vga_gfxd[i] >> 1) & 1, (vga_gfxd[i] >> 0) & 1);
    }
    debug_log(DEBUG_DETAIL, "\r\n");
    for (i = 0; i < 0x19; i++) {
        debug_log(DEBUG_DETAIL, "  CRTC[0x%02X] = %u%u%u%u%u%u%u%u\r\n", i, (vga_crtcd[i] >> 7) & 1,
                  (vga_crtcd[i] >> 6) & 1, (vga_crtcd[i] >> 5) & 1, (vga_crtcd[i] >> 4) & 1, (vga_crtcd[i] >> 3) & 1,
                  (vga_crtcd[i] >> 2) & 1, (vga_crtcd[i] >> 1) & 1, (vga_crtcd[i] >> 0) & 1);
    }
    debug_log(DEBUG_DETAIL, "\r\n");
#endif
}
